...
import scipy
from KDEpy import FFTKDE
from utils.bw_eval import isj, hsilverman
...
def distempcont_bs2(Xd,wd,opt_max_EMP):
    ...
    if plot:
        ...
        plt.figure(1)
        plt.plot(ix,pc,'.',t,fyyt22(t),'--' )
        plt.show()
        plt.hist(Xd,bins=100,density=True)
        plt.plot(t,fd22(t),'--', label='fd22')
        plt.plot(t,fd2(t),'--', label='fd2' )
        plt.plot(t,fyydt2(t),'--', label='fd2' )
        plt.legend()
        plt.show()
    
    
    return fyydt2, fyyt22


# if __name__ == "__main__":
#     Xd = (np.random.randn(2**16))
#     wd=np.ones(len(Xd))
#     distempcont_bs2(Xd,wd,opt_max_EMP={'emp_distr_linear':False,'emp_distr_allbs':True, 'distr_emp_bs2_bw':np.nan,'distemp_bs2_linear':False} )
